...
var
  ...
  pyList:PPyObject;
  i:Integer;
  coords:array[0..41] of Double;
begin
  pyEngine := GetPythonEngine;
  if pyEngine.PyArg_ParseTuple(Args, 'O', @pyList) <> 0 then
  begin
    if pyEngine.PyList_Check(pyList) then
    begin
      for i  := 0 to 41 do
        coords[i] := pyEngine.PyFloat_AsDouble(pyEngine.PyList_GetItem(pyList, i));
      if controlOn = 1 then
      begin
        mouseControl(coords[2], coords[3]);
        if Hypot(coords[8]-coords[16], coords[9]-coords[17]) < clickCoef then
          leftClick();
      end;
    end;
  end;
